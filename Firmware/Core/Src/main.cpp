...
/* USER CODE BEGIN PV */
// Globals
volatile bool foc_enable = false;

PhaseCurrent CurrentX, CurrentY, CurrentZ;
...
void SerialWriteFloat(float value, uint8_t decimal_place = 2) {
    char buf[20];
    char *p = buf;
...
/* USER CODE BEGIN 0 */
MT6816 Encoder;
FOC MotorZ(DRV8311P::ADDRESS::DRV0);
CSA CurrentSensor;
/* USER CODE END 0 */
...
int main(void)
{
  ...
  MX_ADC3_Init();
  MX_DAC1_Init();
  MX_USART1_UART_Init();    // Debugging with TTL
  // MX_SPI1_Init();
  // MX_I2C4_Init();
  // MX_UART4_Init();
  // MX_UART5_Init();
  ...
  /* USER CODE BEGIN 2 */
  // Deselect all Devices in same BUS
  HAL_GPIO_WritePin(SENSOR_NCS1_GPIO_Port, SENSOR_NCS1_Pin, GPIO_PIN_SET);
  HAL_GPIO_WritePin(SENSOR_NCS2_GPIO_Port, SENSOR_NCS2_Pin, GPIO_PIN_SET);
  HAL_GPIO_WritePin(SENSOR_NCS3_GPIO_Port, SENSOR_NCS3_Pin, GPIO_PIN_SET);
  HAL_GPIO_WritePin(SENSOR_NCS4_GPIO_Port, SENSOR_NCS4_Pin, GPIO_PIN_SET);
  ...
  CurrentSensor.Init();
  Encoder.Config(&hspi2);
  Encoder.Init();
  
  
  // Offset Calibrate Code
  // PhaseVoltage tempVolt;
  // tempVolt.A=2; tempVolt.B=-1; tempVolt.C=-1;
  // MotorZ.writeDuty(tempVolt);
  // startDMAmotor();
  // HAL_Delay(100);


  // Timers
  // HAL_TIM_PWM_Start_IT(&htim4);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
  if (GPIO_Pin == MCP_NFAULT_Pin) {
    // Disable FOC
    MotorZ.sleep(); // Sleeps All
    foc_enable = false;
  }
}
/* USER CODE END 4 */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  if (htim->Instance == TIM4) {
    // TIM4 ARR Interrupt Callback. Can be useful
  }
  if (htim->Instance == TIM7) {
    // TIM7 ARR Interrupt Callback. Can be useful
  }
